...
# @depends: none
# @platforms: *
# @tags: filter, median, lowpass, kalman, moving-average, butterworth
...
class RMS(Base):
    ...
    def reset(self) -> None:
        super().reset()
        v = self._initial
        self._buf = array("f", [v] * self._window_size)
        self._sum_of_squares = self._window_size * v * v
        self._idx = 0
...
class Kalman1D(Base):
    def __init__(self, R: float = 25.0, Q: float = 4.0, initial: float = 0.0) -> None:
        ...
        if R <= 0:
            raise FilterConfigurationError("Measurement noise R must be positive")
        if Q < 0:
            raise FilterConfigurationError("Process noise Q must be non-negative")
        
        self._R = float(R)
        self._Q = float(Q)
        self._x = float(initial)
        self._v = 0.0
    ...
    def update(self, z: float, dt: float = 0.06) -> float:
        ...
        self._x += self._v * dt
        P = self._P
        P[0][0] += dt * (2.0 * P[1][0] + dt * P[1][1]) + self._Q
        P[0][1] += dt * P[1][1]
        P[1][0] += dt * P[1][1]
        P[1][1] += self._Q
        ...
        self._x += K0 * innovation
        self._v += K1 * innovation
    ...
    def update_auto_dt(self, z: float) -> float:
        ...
        if self._last_time_us > 0:
            dt = time.ticks_diff(now, self._last_time_us) / 1_000_000.0
        else:
            dt = 0.06
        self._last_time_us = now
        return self.update(z, dt)
