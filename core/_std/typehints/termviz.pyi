...
- ANSI Term support with TrueColor RGB and 256-Term modes
...
- Semantic Term palettes for professional UIs
- Memory-efficient incremental rendering
...
class Canvas:
    ...
    Key features:
    - incremental rendering: Only updates changed regions for efficiency
    - TrueColor (24-bit RGB) and 256-Term palette support
    ...
    """Default Term for cleared pixels."""
    
    color_mode: str
    """Term mode: 'truecolor' (24-bit) or '256' (8-bit palette)."""
    ...
    def __init__(self, cols: int, rows: int,
        ...
        :param default_rgb: Default pixel Term as (r, g, b) tuple or 0xRRGGBB int (default: (220,220,220))
        :param color_mode: Term rendering mode - 'truecolor' for 24-bit RGB or '256' for 8-bit palette (default: 'truecolor')
        ...
        >>> 
        >>> # Canvas with custom default Term
        >>> cv = Canvas(30, 15, default_rgb=(50, 50, 50))  # Dark gray background
        >>> 
        >>> # Using 256-Term mode for compatibility
    ...
    def begin(self) -> None:
        ...
        Resets all pixels to the cleared state (using default_rgb Term).
    ...
    def clear(self) -> None:
        """
        Clear terminal screen and reset incremental rendering cache.

        Outputs a full-screen clear sequence (cursor home + erase display),
        then resets the internal diff buffer so the next render() call
        performs a complete re-render rather than a incremental update.

        Use this when you need a visually clean redraw — most commonly after
        calling set_viewport() to change the plot layout.  Because axis labels
        and tick labels are written directly to terminal cursor positions (not
        through the Braille pixel buffer), the diff renderer cannot erase them
        on the next frame.  clear() wipes the terminal and resets the cache so
        the following show() starts from a blank screen.

        Example
        -------
        ```python
        >>> cv = Canvas(80, 24)
        >>> plt = Plot(cv, xlim=(0, 60), ylim=(0, 100))
        >>> plt.show(clear_after=True)          # initial render (labels cut off)
        >>> # ... wait for key press ...
        >>> cv.clear()                          # blank screen + reset diff cache
        >>> plt.set_viewport(10, 8, 150, 80)   # add proper margins
        >>> plt.plot(xs, ys)
        >>> plt.show()                          # clean re-render, no ghosting
        ```
        """
    ...
    def set_px(self, x: int, y: int, rgb: int) -> None:
        """
        Set pixel at coordinates (x, y) with specified Term.
        
        Turns on the pixel at the given coordinates and assigns it a Term.
        ...
        :param rgb: Packed 24-bit RGB Term value (0xRRGGBB format)
        ...
        ...     Term = (red << 16) | 0x0000FF  # Red to magenta
        ...     cv.set_px(x, 30, Term)
        >>> 
        >>> cv.render()
        ```
    ...
    def clr_px(self, x: int, y: int) -> None:
        ...
        default background Term. Changes take effect after the next render() call.
    ...
    def render(self, ox: int = 1, oy: int = 1) -> None:
        """
        Render canvas to terminal with incremental updates.
        ...
        >>> 
        >>> # Animation loop with efficient incremental updates
    ...
    def sgr(self, rgb: int | tuple[int, int, int] | None) -> str:
        """
        Generate ANSI SGR (Select Graphic Rendition) Term sequence.
        
        Converts an RGB Term value to the appropriate ANSI escape sequence
        based on the canvas color_mode ('truecolor' or '256').
        
        :param rgb: RGB Term as packed int (0xRRGGBB), tuple (r,g,b), or None for default
        :return: ANSI escape sequence string for setting foreground Term
        ...
        >>> print(red_seq + "This text is red" + Term.RESET)
        >>> 
        >>> # Using tuple format
        >>> green_seq = cv.sgr((0, 255, 0))
        >>> print(green_seq + "This text is green" + Term.RESET)
        ```
        """
    ...
    def pack_rgb(rgb: int | tuple[int, int, int]) -> int:
        """
        Pack RGB Term into 24-bit integer format.
        
        Converts Term from tuple (r, g, b) or validates integer format to
        ensure proper 24-bit RGB representation (0xRRGGBB).
        
        :param rgb: RGB Term as tuple (r, g, b) or packed int
        ...
        ```python
        >>> # Pack tuple to int
        >>> Term = Canvas.pack_rgb((255, 128, 64))
        >>> print(hex(Term))  # Output: 0xff8040
        >>> 
        >>> # Validate/normalize int
        >>> Term = Canvas.pack_rgb(0xFF8040)
        >>> print(hex(Term))  # Output: 0xff8040
        >>> 
        >>> # Use in drawing
        >>> cv = Canvas(40, 20)
...
class Plot:
    ...
    - Term cycling for multiple datasets
    ...
    def title(self, s: str | None = None, Term: tuple[int, int, int] = (230, 230, 230)) -> None:
        """
        Set plot title text and Term.
        ...
        :param Term: Text Term as RGB tuple (default: (230, 230, 230))
        ...
        >>> 
        >>> # Custom Term
        >>> plt.title("Temperature", Term=(255, 100, 100))
        ```
        """
    
    def xlabel(self, s: str | None = None, Term: tuple[int, int, int] = (210, 210, 210)) -> None:
        """
        Set X-axis label text and Term.
        ...
        :param Term: Text Term as RGB tuple (default: (210, 210, 210))
        ...
        >>> 
        >>> # With custom Term
        >>> plt.xlabel("Frequency (Hz)", Term=(100, 200, 255))
        ```
        """
    
    def ylabel(self, s: str | None = None, Term: tuple[int, int, int] = (210, 210, 210)) -> None:
        """
        Set Y-axis label text and Term.
        ...
        >>> 
        >>> # With units
        >>> plt.ylabel("Temperature (°C)", Term=(255, 128, 64))
        ```
        """
    ...
    def set_legend_colors(self, colors: list[tuple[int, int, int]]) -> None:
        ...
        Sets custom colors for legend entries, overriding the Term cycle.
        Useful for matching legend to specific Term schemes.
    ...
    def set_legend_items(self, names: list[str]) -> None:
        ...
        the Term cycle or custom colors if set with set_legend_colors().
    ...
    def grid(self, flag: bool | None = None) -> bool | None:
        ...
        lines at Y-axis ticks. Grid is drawn in a subdued Term to avoid
    ...
    def plot(self, *args, label: str | None = None, Term: tuple[int, int, int] | None = None) -> None:
        """
        ...
        :param Term: Line Term as RGB tuple. If None, uses next Term from Term cycle (default: None)
        ...
        >>> plt.plot(y, label="sin", Term=(255, 0, 0))
        >>> 
        >>> # Explicit X and Y
        >>> x = [i * 0.1 for i in range(100)]
        >>> y = [math.cos(v) for v in x]
        >>> plt.plot(x, y, label="cos", Term=(0, 255, 0))
        >>> 
        >>> plt.legend()
        >>> plt.show()
        ```
    ...
    def scatter(self, x, y=None, size: int = 1, label: str | None = None,
        ...
        :param size: Marker size as diamond radius. size=1: single dot, size=2: 5-dot cross, size=3: 13-dot diamond (default: 1)
    ...
    def text(self, x: float, y: float, text: str, color: tuple[int, int, int] = (220, 220, 220)) -> None:
        """
    ...
    def polygon(self, vertices, label: str | None = None,
                color: tuple[int, int, int] | None = None,
                fill: bool = False) -> None:
        """
        Draw polygon from a list of vertices.

        Connects each adjacent pair of vertices with a line segment and closes
        the shape back to the first vertex. Coordinates are in data space.
        Can be drawn as outline or filled using a scanline algorithm.

        :param vertices: Sequence of (x, y) tuples in data coordinates
        :param label: Legend label for this polygon (default: None)
        :param color: Polygon color as RGB tuple. If None, uses next color from color cycle (default: None)
        :param fill: If True, fills the polygon interior; if False, draws outline only (default: False)
        ...
        >>> 
        >>> # Outline triangle
        >>> plt.polygon([(2, 1), (8, 1), (5, 8)], color=(100, 200, 255), label="zone")
        >>> 
        >>> # Filled warning zone
        >>> plt.polygon([(0, 7), (10, 7), (10, 10), (0, 10)],
        ...             color=(220, 80, 80), fill=True)
        >>> 
        >>> plt.legend()
        >>> plt.show()
        ```
    ...
    def show(self, clear_after: bool = False) -> None:
        ...
        >>> x = list(range(11))
        >>> y = [i**2 for i in x]
        >>> plt.plot(x, y, label="y=x²", Term=(255, 128, 0))
...
class Scope:
    ...
    (grid, axes, labels), and efficient incremental rendering.
    ...
    def set_colors(self, colors: list[tuple[int, int, int]]) -> None:
        ...
        Sets custom colors for each channel, replacing the default Term
        cycle. If there are more channels than colors, colors will be
        reused cyclically.
        
        :param colors: List of RGB Term tuples, one per channel
    ...
    def text(self, x: float, y: float, text: str,
             Term: tuple[int, int, int] = (200, 200, 200), align: str = 'left') -> None:
        """
        ...
        :param y: Y coordinate in data space
        :param text: Text string to display
        :param Term: Text Term as RGB tuple (default: (200, 200, 200))
        ...
        >>> scope.text(5, 1.5, "High", Term=(255, 100, 100))
        >>> scope.text(5, -1.5, "Low", Term=(100, 100, 255))
        >>> scope.text(60, 0, "Zero", Term=(200, 200, 200), align='center')
        >>> 
        >>> # Start plotting
