"""
ufilter - Signal Processing Filter Library
==========================================

A collection of signal processing filters for MicroPython embedded systems.
All filters inherit from Base class and provide consistent API: `update(x)`, `reset()`, etc.
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ "I want to smooth noisy sensor readings"                                    │
│   → Alpha, LowPass, MovingAverage                                          │
│                                                                             │
│ "I want to remove sudden spikes/outliers"                                   │
│   → Median                                                                  │
│                                                                             │
│ "I want to limit rate of change" (motor acceleration, etc.)                 │
│   → SlewRateLimiter                                                         │
│                                                                             │
│ "I want to track/predict a moving object" (position + velocity)             │
│   → Kalman1D                                                                │
│                                                                             │
│ "I want optimal noise reduction for static sensors"                         │
│   → Kalman                                                                  │
│                                                                             │
│ "I want to remove slow drift and detect only fast changes"                  │
│   → HighPass                                                                │
│                                                                             │
│ "I want to measure RMS power/amplitude"                                     │
│   → RMS                                                                     │
│                                                                             │
│ "I want auto-adjustment for varying noise conditions"                       │
│   → Adaptive                                                                │
│                                                                             │
│ "I want to filter compass/IMU angles" (0°↔360° wrap-around)                 │
│   → AngleEMA                                                                │
│                                                                             │
│ "I want to control motor/temperature to a setpoint"                         │
│   → PID                                                                     │
│                                                                             │
│ "I want to chain multiple filters sequentially"                             │
│   → FilterChain                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
1. Smoothing Filters (Noise Reduction)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

┌──────────────┬───────────────────────────────────────────────────────────────┐
│ Alpha        │ Simplest exponential moving average (EMA)                     │
│              │ - Params: alpha (0~1, higher = faster response)               │
│              │ - Pros: Minimal memory (1 state), fast computation            │
│              │ - Use: General sensor noise reduction                         │
│              │ - Example: Temperature, light, voltage sensors                │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ LowPass      │ Frequency-based low-pass filter                               │
│              │ - Params: fc (cutoff freq Hz), fs (sampling freq Hz)          │
│              │ - Pros: Physically meaningful ("remove above 10Hz")           │
│              │ - Use: Remove noise above specific frequency                  │
│              │ - Example: Accelerometer 100Hz sampling, remove >5Hz noise    │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ TauLowPass   │ Time-constant (τ) based low-pass filter                       │
│              │ - Params: tau (time constant sec), mode='auto'/'fixed'        │
│              │ - Pros: Supports variable sampling intervals (auto mode)      │
│              │ - Use: Data arriving at irregular intervals                   │
│              │ - Example: Network-received sensor data                       │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ MovingAverage│ Simple Moving Average (SMA)                                   │
│              │ - Params: window_size                                         │
│              │ - Pros: Predictable phase delay, intuitive                    │
│              │ - Use: When you need average over fixed window                │
│              │ - Example: Average of last 10 samples                         │
└──────────────┴───────────────────────────────────────────────────────────────┘

2. Outlier Removal / Signal Limiting
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

┌──────────────┬───────────────────────────────────────────────────────────────┐
│ Median       │ Median filter                                                 │
│              │ - Params: window_size (odd recommended)                       │
│              │ - Pros: Robust against spikes/impulse noise                   │
│              │ - Use: Sensors with occasional outliers                       │
│              │ - Example: Ultrasonic distance sensor (occasional spikes)     │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ SlewRateLimiter│ Rate of change limiter                                      │
│              │ - Params: max_rate or (rise_rate, fall_rate)                  │
│              │ - Pros: Prevents sudden changes, asymmetric rise/fall         │
│              │ - Use: Motor acceleration limits, smooth transitions          │
│              │ - Example: Robot servo speed limiting                         │
└──────────────┴───────────────────────────────────────────────────────────────┘

3. State Estimation (Kalman Filters)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

┌──────────────┬───────────────────────────────────────────────────────────────┐
│ Kalman       │ 1D Kalman filter (position only)                              │
│              │ - State: position                                             │
│              │ - Params: R (measurement noise), Q (process noise)            │
│              │ - Use: Static or slowly changing value estimation             │
│              │ - Example: Thermometer, barometer noise reduction             │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ Kalman1D     │ 1D Kalman filter (position + velocity)                        │
│              │ - State: position, velocity                                   │
│              │ - Params: R, Q, dt (sampling interval)                        │
│              │ - Use: Moving object tracking and prediction                  │
│              │ - Example: Ultrasonic sensor tracking moving object           │
└──────────────┴───────────────────────────────────────────────────────────────┘
...
│              │ - Params: fc (cutoff freq Hz), fs (sampling freq Hz)          │
│              │ - Use: Remove DC/drift, detect only changes                   │
│              │ - Example: Remove gravity from accelerometer, detect vibration│
├──────────────┼───────────────────────────────────────────────────────────────┤
│ Biquad       │ 2nd order IIR filter (direct coefficients)                    │
│              │ - Params: b0, b1, b2, a1, a2 (filter coefficients)            │
│              │ - Use: Custom filter response needed                          │
│              │ - Example: Audio equalizer, specific frequency band filter    │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ Butterworth  │ Butterworth filter (maximally flat response)                  │
│              │ - Params: fc, fs, filter_type ('low'/'high')                  │
│              │ - Pros: Flat passband response                                │
│              │ - Use: Clean frequency cutoff needed                          │
│              │ - Example: Audio signal processing                            │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ FIR          │ Finite Impulse Response filter                                │
│              │ - Params: taps (filter coefficient list)                      │
│              │ - Pros: Linear phase, always stable                           │
│              │ - Use: Precise filter response needed                         │
│              │ - Example: High-quality audio processing                      │
└──────────────┴───────────────────────────────────────────────────────────────┘

5. Special Purpose Filters
~~~~~~~~~~~~~~~~~~~~~~~~~~

┌──────────────┬───────────────────────────────────────────────────────────────┐
│ RMS          │ Root Mean Square measurement                                  │
│              │ - Params: window_size                                         │
│              │ - Use: Signal power/amplitude, vibration magnitude monitoring │
│              │ - Example: AC current measurement, vibration motor intensity  │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ Adaptive     │ Auto-adjusting smoothing filter                               │
│              │ - Params: min_alpha, max_alpha, sensitivity                   │
│              │ - Pros: Auto-adjusts alpha based on signal changes            │
│              │ - Use: Environments with varying noise characteristics        │
│              │ - Example: Moving robot sensors (stationary↔moving)           │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ AngleEMA     │ Exponential moving average for angles                         │
│              │ - Params: alpha, mode ('rad'/'deg')                           │
│              │ - Pros: Handles 0°↔360° (or -π↔π) wrap-around                 │
│              │ - Use: Compass, gyro, IMU heading filtering                   │
│              │ - Example: Correct averaging at 359°→1° transition            │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ PID          │ PID Controller                                                │
│              │ - Params: Kp, Ki, Kd, setpoint, limits                        │
│              │ - Features: anti-windup, derivative filter, output limits     │
│              │ - Use: Setpoint tracking control                              │
│              │ - Example: Temperature control, motor speed, line follower    │
├──────────────┼───────────────────────────────────────────────────────────────┤
│ FilterChain  │ Filter chain (serial connection)                              │
│              │ - Params: filters (list of filter instances)                  │
│              │ - Use: Apply multiple filters sequentially                    │
│              │ - Example: Median → LowPass → SlewRateLimiter                 │
└──────────────┴───────────────────────────────────────────────────────────────┘
...
```python
from ufilter import Alpha, Median, LowPass, Kalman1D, FilterChain

# 1. Simple noise reduction
f = Alpha(alpha=0.2)
filtered = f.update(raw_sensor_value)

# 2. Spike removal then smoothing
f = FilterChain([Median(5), Alpha(0.3)])
filtered = f.update(raw_value)

# 3. Frequency-based filtering (100Hz sampling, remove >5Hz)
f = LowPass(fc=5, fs=100)
filtered = f.update(accelerometer_x)

# 4. Moving object tracking (position + velocity estimation)
f = Kalman1D(R=5.0, Q=0.1, dt=0.01)
f.update(distance_cm)
print(f"Position: {f.position:.1f}, Velocity: {f.velocity:.1f}")
```


Common API (All Filters)
------------------------

All filters inherit from Base class and provide these methods:

- `update(x)` → float: Input new sample, return filtered value
- `reset()`: Reset filter state
- `value` (property): Current filter output value
- `process(data)` → list: Batch processing
...
class Kalman1D(Base):
    """
    1D Kalman filter with position and velocity state for tracking moving objects.
    
    A two-state Kalman filter that tracks both position and velocity, making it
    ideal for estimating the state of moving objects. Unlike the scalar Kalman
    filter which only tracks position, this filter uses a constant-velocity model
    to predict future states based on current velocity.
    ...
    Key Features:
        ...
        - Two-state tracking (position and velocity)
        - Time-dependent prediction (uses dt)
        - Velocity estimation for motion prediction
        - 2x2 covariance matrix for complete uncertainty tracking
        - Suitable for distance sensors, GPS, motion tracking
    
    Applications:
    
        - Ultrasonic distance sensor filtering
        - GPS position/velocity tracking
        - Robot localization
        - Object tracking in motion control
        - Any sensor measuring moving targets
    
    Comparison with Kalman:
    
        | Feature          | Kalman         | Kalman1D               |
        |------------------|----------------|------------------------|
        | State variables  | 1 (position)   | 2 (position + velocity)|
        | Covariance       | Scalar         | 2x2 matrix             |
        | Time dependency  | None           | Requires dt            |
        | Prediction model | x_pred = x     | x_pred = x + v*dt      |
        | Use case         | Static values  | Moving objects         |
    """
    
    def __init__(self, R: float = 25.0, Q: float = 4.0, initial: float = 0.0) -> None:
        """
        Initialize 1D Kalman filter with position-velocity state.
        
        :param R: Measurement noise variance - how noisy the sensor is.
                  Higher values make filter trust predictions more than measurements.
                  Typical: 10-100 for ultrasonic sensors (default: 25.0)
        :param Q: Process noise variance - how much the true state can change.
                  Higher values make filter more responsive to changes.
                  Typical: 1-10 for moderate dynamics (default: 4.0)
        :param initial: Initial position estimate (default: 0.0)
        
        :raises FilterConfigurationError: If R <= 0 or Q < 0
        
        Parameter Selection Guidelines:
        
            R (Measurement Noise):
                - Low (1-10): Trust sensor readings (precise sensors)
                - Medium (10-50): Moderate trust
                - High (50-200): Distrust sensor (noisy environment)
            
            Q (Process Noise):
                - Low (0.1-1): Smooth tracking (slow-moving targets)
                - Medium (1-10): Moderate responsiveness
                - High (10-50): Fast tracking (quick-moving targets)
        ...
        # Ultrasonic distance sensor filtering
        filter = Kalman1D(R=25.0, Q=4.0, initial=50.0)
        
        # Process measurements with timing
        import time
        last_time = time.ticks_us()
        
        while True:
            raw_distance = sensor.read()
            now = time.ticks_us()
            dt = time.ticks_diff(now, last_time) / 1_000_000.0
            last_time = now
            
            filtered = filter.update(raw_distance, dt)
            print(f"Distance: {filtered:.1f} cm, Velocity: {filter.velocity:.2f} cm/s")
        ```
        """
    
    @micropython.native
    def update(self, z: float, dt: float = 0.06) -> float:
        """
        Update filter with new measurement and time delta.
        
        Performs prediction and update steps of the Kalman filter using
        the specified time interval for motion prediction.
        
        :param z: New measurement value (e.g., distance in cm)
        :param dt: Time since last update in seconds (default: 0.06 = 60ms)
        :return: Filtered position estimate
        
        Algorithm:
        
            1. Predict: x = x + v*dt, update covariance P
            2. Update: Calculate Kalman gain, correct position and velocity
        ...
        # Known 50ms interval
        filtered = filter.update(raw_value, dt=0.05)
        
        # Variable interval (calculate dt)
        now = time.ticks_us()
        dt = time.ticks_diff(now, last_time) / 1_000_000.0
        filtered = filter.update(raw_value, dt)
        ```
        """
    
    def update_auto_dt(self, z: float) -> float:
        """
        Update filter with automatic time delta calculation.
        
        Automatically calculates dt using time.ticks_us() for convenience
        when manual timing is not required.
        
        :param z: New measurement value
        :return: Filtered position estimate
        
        Note:
            First call uses default dt=0.06s since no previous timestamp exists.
        ...
        # Simple usage without manual timing
        while True:
            raw = sensor.read()
            filtered = filter.update_auto_dt(raw)
    ...
    def position(self) -> float:
        """Current position estimate (same as filter output)."""
        ...
    
    @property
    def velocity(self) -> float:
        """Current velocity estimate (change rate of position)."""
        ...
    ...
    def reset(self) -> None:
        ...
        Clears position, velocity, covariance matrix, and timing state.
        Preserves R and Q parameters.
        """
        ...
