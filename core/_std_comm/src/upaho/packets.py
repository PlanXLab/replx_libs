...
from .properties import (
    ...
    _encode_utf8, _decode_utf8, _encode_binary
)
...
class PublishPacket(MQTTPacket):
    def __init__(self, topic, payload=None, qos=0, retain=False, dup=False,
        ...
        self.mid = mid
        self.protocol_version = protocol_version
        self.properties = properties if properties else Properties.empty()
    ...
    def pack(self):
        ...
        if isinstance(self.payload, str):
            variable_header.extend(self.payload.encode('utf-8'))
        else:
            variable_header.extend(self.payload)
    ...
    def unpack(flags, data, protocol_version=MQTTProtocolVersion.MQTTv5):
        ...
        packet.payload = bytes(memoryview(data)[offset:])  # avoids intermediate bytearray slice copy
...
class PubackPacket(MQTTPacket):
    ...
    def pack(self):
        ...
        # MQTT v5: Omit reason code and properties if SUCCESS with no properties
        if self.protocol_version == MQTTProtocolVersion.MQTTv5:
            has_properties = self.properties._properties if hasattr(self.properties, '_properties') else False
            if self.reason_code != ReasonCode.SUCCESS or has_properties:
...
class DisconnectPacket(MQTTPacket):
    ...
    def pack(self):
        ...
        # MQTT v5: Omit reason code and properties if NORMAL_DISCONNECTION with no properties
        has_properties = self.properties._properties if hasattr(self.properties, '_properties') else False
        if self.reason_code == ReasonCode.NORMAL_DISCONNECTION and not has_properties:
            return self._pack_fixed_header(0)
