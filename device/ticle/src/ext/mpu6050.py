__version__ = "1.0.0"
__author__ = "PlanX Lab Development Team"

from . import (
    math, utime,
    I2CMaster
)


class MPU6050:
    _RA_SMPLRT_DIV      = 0x19
    _RA_CONFIG          = 0x1A
    _RA_GYRO_CONFIG     = 0x1B
    _RA_ACCEL_CONFIG    = 0x1C
    _RA_FIFO_EN         = 0x23
    _RA_INT_ENABLE      = 0x38
    _RA_INT_STATUS      = 0x3A
    _RA_ACCEL_XOUT_H    = 0x3B
    _RA_TEMP_OUT_H      = 0x41
    _RA_GYRO_XOUT_H     = 0x43
    _RA_SIGNAL_PATH_RST = 0x68
    _RA_USER_CTRL       = 0x6A
    _RA_PWR_MGMT_1      = 0x6B
    _RA_PWR_MGMT_2      = 0x6C
    _RA_BANK_SEL        = 0x6D
    _RA_MEM_START_ADDR  = 0x6E
    _RA_MEM_R_W         = 0x6F
    _RA_FIFO_COUNTH     = 0x72
    _RA_FIFO_R_W        = 0x74
    _RA_WHO_AM_I        = 0x75

    _BIT_DMP_EN         = 7
    _BIT_FIFO_EN        = 6
    _BIT_DMP_RST        = 3
    _BIT_FIFO_RST       = 2

    _ACC_LSB_PER_G      = 4096.0                # ±8g
    _G0                 = 9.80665               # [m/s^2]
    _GYR_LSB_PER_DPS    = 16.4                  # ±2000 dps
    _DPS_TO_RAD_S       = 0.017453292519943295  # math.pi / 180.0
    _DMP_PACKET_SIZE    = 42                    # include quat (16 bytes)

    _RA_DMP_CFG_1 = 0x70
    _RA_DMP_CFG_2 = 0x71

    _DMP_FW_BYTES = bytes([
        # bank 0, 256 bytes
        ,

        # bank 1, 256 bytes
        ,

        # bank 2, 256 bytes
        ,

        # bank 3, 256 bytes
        ,

        # bank 4, 256 bytes
        ,

        # bank 5, 256 bytes
        ,

        # bank 6, 256 bytes
        ,

        # bank 7, 138 bytes (remainder)
        
    ])

    _DMP_CFG_BYTES = bytes([
        # [BANK, OFFSET, LENGTH, <DATA...>]
        ,

        # SPECIAL 0x01 = enable interrupts  (bank=0, offset=0, length=0, data=0x01)
        0x00, 0x00, 0x00, 0x01,

        0x07, 0x86, 0x01, 0xFE,
        0x07, 0x41, 0x05, 0xF1, 0x20, 0x28, 0x30, 0x38,
        0x07, 0x7E, 0x01, 0x30,
        0x07, 0x46, 0x01, 0x9A,
        0x07, 0x47, 0x04, 0xF1, 0x28, 0x30, 0x38,
        0x07, 0x6C, 0x04, 0xF1, 0x28, 0x30, 0x38,
        0x02, 0x16, 0x02, 0x00, 0x01
    ])

    _DMP_UPD_BYTES = bytes([
        0x01, 0xB2, 0x02, 0xFF, 0xFF,
        0x01, 0x90, 0x04, 0x09, 0x23, 0xA1, 0x35,
        0x01, 0x6A, 0x02, 0x06, 0x00,
        0x01, 0x60, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x60, 0x04, 0x40, 0x00, 0x00, 0x00,
        0x01, 0x62, 0x02, 0x00, 0x00,
        0x00, 0x60, 0x04, 0x00, 0x40, 0x00, 0x00
    ])

    DLPF_DIV8K_256HZ       = 0   # Minimum filtering, 8kHz internal sample rate
    DLPF_DIV1K_98HZ        = 2   # Stronger filtering, 1kHz internal sample rate
    DLPF_DIV1K_42HZ        = 3   # Even stronger filtering, 1kHz internal sample rate
    SIMPLERT_DIV8K_1KHz    = 7
    SIMPLERT_DIV1K_200Hz   = 4
    SIMPLERT_DIV1K_100Hz   = 9

    class Mode:
        DMP_STABLE   = "dmp_stable"
        DMP_FAST     = "dmp_fast"
        RAW_BALANCED = "raw_balanced"
        RAW_FAST     = "raw_fast"

    _MODE_PRESETS = {
        Mode.DMP_STABLE:   (DLPF_DIV1K_42HZ,  SIMPLERT_DIV1K_100Hz, True),
        Mode.DMP_FAST:     (DLPF_DIV1K_98HZ,  SIMPLERT_DIV1K_200Hz, True),
        Mode.RAW_BALANCED: (DLPF_DIV1K_42HZ,  SIMPLERT_DIV1K_200Hz, False),
        Mode.RAW_FAST:     (DLPF_DIV8K_256HZ, SIMPLERT_DIV8K_1KHz,  False),
    }

    def __init__(self, scl, sda, *, addr=0x68, mode=Mode.RAW_BALANCED):
        self._i2c  = I2CMaster(sda=sda, scl=scl)
        self._addr = int(addr)
        self._pkt_size = int(self._DMP_PACKET_SIZE)
        self._heading_offset = 0.0

        # Reset & Wake
        self._i2c.write_u8(self._addr, self._RA_PWR_MGMT_1, 0x80)
        utime.sleep_ms(100)
        self._i2c.write_u8(self._addr, self._RA_SIGNAL_PATH_RST, 0x07)
        utime.sleep_ms(10)
        self._i2c.write_u8(self._addr, self._RA_PWR_MGMT_1, 0x01)
        self._i2c.write_u8(self._addr, self._RA_PWR_MGMT_2, 0x00)
        utime.sleep_ms(10)

        who = self._i2c.read_u8(self._addr, self._RA_WHO_AM_I)
        if who not in (0x68, 0x69, 0x70):
            raise RuntimeError("MPU6050 not found (WHO_AM_I=0x%02X)" % who)

        if mode not in self._MODE_PRESETS:
            raise ValueError("Unknown mode: %r" % (mode,))
        dlpf, smplrt_div, use_dmp = self._MODE_PRESETS[mode]
        self._use_dmp = bool(use_dmp)

        # Ranges/DLPF/Sample rate
        self._i2c.write_u8(self._addr, self._RA_CONFIG, dlpf)
        cur = self._i2c.read_u8(self._addr, self._RA_GYRO_CONFIG)
        self._i2c.write_u8(self._addr, self._RA_GYRO_CONFIG, (cur & ~0x18) | (3 << 3))   # ±2000 dps
        cur = self._i2c.read_u8(self._addr, self._RA_ACCEL_CONFIG)
        self._i2c.write_u8(self._addr, self._RA_ACCEL_CONFIG, (cur & ~0x18) | (2 << 3))  # ±8 g
        self._i2c.write_u8(self._addr, self._RA_SMPLRT_DIV, smplrt_div)

        # Disable INT/FIFO/DMP initially
        self._i2c.write_u8(self._addr, self._RA_INT_ENABLE, 0x00)
        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, 0x00)
        self._i2c.write_u8(self._addr, self._RA_FIFO_EN, 0x00)

        self._acc_bias = [0.0, 0.0, 0.0]  # m/s^2
        self._gyr_bias = [0.0, 0.0, 0.0]  # rad/s
        self._buf6 = bytearray(6)
        self._buf42 = bytearray(self._pkt_size)

        self._quick_bias()

        if self._use_dmp:
            self._load_dmp_firmware()
            self._flush_fifo()
            self._wait_for_dmp_packet()
            self.zero_heading()

    def deinit(self):
        self._i2c.write_u8(self._addr, self._RA_INT_ENABLE, 0x00)
        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, 0x00)
        self._i2c.write_u8(self._addr, self._RA_FIFO_EN, 0x00)
        utime.sleep_ms(10)
        self._i2c.write_u8(self._addr, self._RA_PWR_MGMT_1, 0x40)
        try: 
            self._i2c.deinit()
        except Exception: 
            pass

    @property
    def accel(self):
        self._i2c.readfrom_mem_into(self._addr, self._RA_ACCEL_XOUT_H, self._buf6)
        b = self._buf6
        ax = self._twos16(b[0],b[1])
        ay = self._twos16(b[2],b[3])
        az = self._twos16(b[4],b[5])
        # LSB→g→m/s^2
        ax_ms2 = (ax / self._ACC_LSB_PER_G) * self._G0
        ay_ms2 = (ay / self._ACC_LSB_PER_G) * self._G0
        az_ms2 = (az / self._ACC_LSB_PER_G) * self._G0
        return (ax_ms2 - self._acc_bias[0], ay_ms2 - self._acc_bias[1], az_ms2 - self._acc_bias[2])

    @property
    def gyro(self):
        self._i2c.readfrom_mem_into(self._addr, self._RA_GYRO_XOUT_H, self._buf6)
        b = self._buf6
        gx = self._twos16(b[0],b[1])
        gy = self._twos16(b[2],b[3])
        gz = self._twos16(b[4],b[5])
        # LSB→dps→rad/s
        gxd = (gx / self._GYR_LSB_PER_DPS) * self._DPS_TO_RAD_S
        gyd = (gy / self._GYR_LSB_PER_DPS) * self._DPS_TO_RAD_S
        gzd = (gz / self._GYR_LSB_PER_DPS) * self._DPS_TO_RAD_S
        return (gxd - self._gyr_bias[0], gyd - self._gyr_bias[1], gzd - self._gyr_bias[2])

    @property
    def quat(self):
        if not self._use_dmp:
            return (1.0, 0.0, 0.0, 0.0)

        TIMEOUT_MS = 150
        t0 = utime.ticks_ms()
        while True:
            st = self._i2c.read_u8(self._addr, self._RA_INT_STATUS)
            if st & 0x10:      # FIFO overflow
                self._flush_fifo()
            elif st & 0x02:    # DMP data-ready
                break
            if self._fifo_count_direct() >= self._pkt_size:
                break
            if utime.ticks_diff(utime.ticks_ms(), t0) >= TIMEOUT_MS:
                raise RuntimeError("No DMP packet ready")
            utime.sleep_ms(1)

        cnt = self._fifo_count_direct()
        rem = cnt % self._pkt_size
        if rem:
            _ = self._fifo_read_direct(rem); cnt -= rem
        while cnt > self._pkt_size:
            _ = self._fifo_read_direct(self._pkt_size); cnt -= self._pkt_size

        pkt = self._fifo_read_direct(self._pkt_size)
        if len(pkt) != self._pkt_size:
            raise RuntimeError("Short DMP packet")

        q0 = self._twos32(pkt[0], pkt[1], pkt[2], pkt[3])
        q1 = self._twos32(pkt[4], pkt[5], pkt[6], pkt[7])
        q2 = self._twos32(pkt[8], pkt[9], pkt[10], pkt[11])
        q3 = self._twos32(pkt[12], pkt[13], pkt[14], pkt[15])
        if not (q0 | q1 | q2 | q3):
            if self._wait_for_dmp_packet():
                return self.quat
            return (1.0, 0.0, 0.0, 0.0)

        s = 1.0 / (1 << 30)
        return self._q_normalize((q0*s, q1*s, q2*s, q3*s))

    @property
    def euler(self):
        if not self._use_dmp:
            return 0.0, 0.0, 0.0
        
        roll, pitch, yaw = self._quat_to_euler(self.quat)
        if self._heading_offset != 0.0:
            yaw -= self._heading_offset
            if yaw >  math.pi: yaw -= 2*math.pi
            if yaw < -math.pi: yaw += 2*math.pi
        return roll, pitch, yaw

    @property
    def linear(self):
        if not self._use_dmp:
            return (0.0, 0.0, 0.0)
        
        ax, ay, az = self.accel                     # m/s^2
        R = self._rotmat_from_quat_wxyz(self.quat)  # body->world
        axw = R[0][0]*ax + R[0][1]*ay + R[0][2]*az
        ayw = R[1][0]*ax + R[1][1]*ay + R[1][2]*az
        azw = R[2][0]*ax + R[2][1]*ay + R[2][2]*az
        # Gravity compensation: subtract G0 based on m/s^2
        return (axw, ayw, azw - self._G0)

    def zero_heading(self):
        _, _, yaw = self.euler
        self._heading_offset = yaw

    def _flush_fifo(self):
        user = self._i2c.read_u8(self._addr, self._RA_USER_CTRL)
        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, user | (1 << self._BIT_FIFO_RST))
        utime.sleep_ms(2)

    def _wait_for_dmp_packet(self, timeout_ms=150):
        t0 = utime.ticks_ms()
        while utime.ticks_diff(utime.ticks_ms(), t0) < timeout_ms:
            if self._fifo_count_direct() >= self._pkt_size:
                return True
        return False

    def _load_dmp_firmware(self, fw=None, cfg=None, upd=None):
        fw  = fw if fw  is not None else self._DMP_FW_BYTES
        cfg = cfg if cfg is not None else self._DMP_CFG_BYTES
        upd = upd if upd is not None else self._DMP_UPD_BYTES

        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, (1<<self._BIT_DMP_RST) | (1<<self._BIT_FIFO_RST))
        utime.sleep_ms(10)

        self._write_memory_block_direct(fw, bank=0, address=0)
        self._write_config_set_direct(cfg)
        self._write_config_set_direct(upd)

        self._i2c.write_u8(self._addr, self._RA_DMP_CFG_1, 0x03)
        self._i2c.write_u8(self._addr, self._RA_DMP_CFG_2, 0x00)

        user = self._i2c.read_u8(self._addr, self._RA_USER_CTRL)
        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, user | (1<<self._BIT_FIFO_RST))
        utime.sleep_ms(5)

        user |= (1<<self._BIT_DMP_EN) | (1<<self._BIT_FIFO_EN)
        self._i2c.write_u8(self._addr, self._RA_USER_CTRL, user)
        self._i2c.write_u8(self._addr, self._RA_FIFO_EN, 0x00)

        self._pkt_size = int(self._DMP_PACKET_SIZE)
        return True

    def _select_bank_direct(self, bank):
        self._i2c.write_u8(self._addr, self._RA_BANK_SEL, bank & 0x1F)

    def _set_mem_addr_direct(self, addr):
        self._i2c.write_u8(self._addr, self._RA_MEM_START_ADDR, addr & 0xFF)

    def _write_memory_block_direct(self, data: bytes, bank=0, address=0, chunk=16):
        i = 0
        L = len(data)
        while i < L:
            self._select_bank_direct(bank)
            self._set_mem_addr_direct(address)
            space = 256 - address
            this = min(chunk, space, L - i)
            self._i2c.writeto_mem(self._addr, self._RA_MEM_R_W, data[i:i+this])
            i += this
            address += this
            if address == 256:
                bank += 1
                address = 0

    def _write_config_set_direct(self, cfg: bytes):
        i, L = 0, len(cfg)
        while i < L:
            if i + 3 > L:
                raise ValueError("Malformed DMP config (truncated header)")
            
            bank = cfg[i]; offset = cfg[i+1]
            length = cfg[i+2]; i += 3

            if length == 0:
                if i >= L:
                    raise ValueError("Malformed DMP config (missing special code)")
                
                special = cfg[i]
                i += 1
                if special == 0x01:
                    self._i2c.write_u8(self._addr, self._RA_INT_ENABLE, 0x02)  # DMP INT
                continue

            if i + length > L:
                raise ValueError("Malformed DMP config (length overflow)")
            
            self._write_memory_block_direct(cfg[i:i+length], bank=bank, address=offset)
            i += length

    def _fifo_count_direct(self):
        b = self._i2c.readfrom_mem(self._addr, self._RA_FIFO_COUNTH, 2)
        return (b[0] << 8) | b[1]

    def _fifo_read_direct(self, n):
        mv = memoryview(self._buf42)[:n]
        self._i2c.readfrom_mem_into(self._addr, self._RA_FIFO_R_W, mv)
        return mv

    def _quick_bias(self, n=200, delay_ms=2):
        # throw away first readings
        for _ in range(20):
            _ = self._i2c.readfrom_mem(self._addr, self._RA_ACCEL_XOUT_H, 14); utime.sleep_ms(1)

        ax = ay = az = gx = gy = gz = 0.0
        for _ in range(n):
            b = self._i2c.readfrom_mem(self._addr, self._RA_ACCEL_XOUT_H, 14)
            ax += self._twos16(b[0],b[1])
            ay += self._twos16(b[2],b[3])
            az += self._twos16(b[4],b[5])
            gx += self._twos16(b[8],b[9])
            gy += self._twos16(b[10],b[11])
            gz += self._twos16(b[12],b[13])
            utime.sleep_ms(delay_ms)

        inv = 1.0/n
        # accel bias in m/s^2 (Assuming a flat surface: +1g at +Z)
        axg, ayg, azg = (ax*inv)/self._ACC_LSB_PER_G, (ay*inv)/self._ACC_LSB_PER_G, (az*inv)/self._ACC_LSB_PER_G
        self._acc_bias = [axg * self._G0, ayg * self._G0, (azg - 1.0) * self._G0]

        # gyro bias in **rad/s**
        gxd = (gx*inv)/self._GYR_LSB_PER_DPS * self._DPS_TO_RAD_S
        gyd = (gy*inv)/self._GYR_LSB_PER_DPS * self._DPS_TO_RAD_S
        gzd = (gz*inv)/self._GYR_LSB_PER_DPS * self._DPS_TO_RAD_S
        self._gyr_bias = [gxd, gyd, gzd]

    @staticmethod
    def _quat_to_euler(q):
        w, x, y, z = q
        roll = math.atan2(2*(w*x + y*z), 1 - 2*(x*x + y*y))
        sp = 2*(w*y - z*x)
        sp = -1.0 if sp < -1.0 else (1.0 if sp > 1.0 else sp)
        pitch = math.asin(sp)
        yaw = math.atan2(2*(w*z + x*y), 1 - 2*(y*y + z*z))
        return roll, pitch, yaw

    @staticmethod
    def _rotmat_from_quat_wxyz(q):
        w,x,y,z = q
        xx,yy,zz = x*x,y*y,z*z
        wx,wy,wz = w*x,w*y,w*z
        xy,xz,yz = x*y,x*z,y*z
        return [
            [1-2*(yy+zz), 2*(xy-wz), 2*(xz+wy)],
            [2*(xy+wz), 1-2*(xx+zz), 2*(yz-wx)],
            [2*(xz-wy), 2*(yz+wx), 1-2*(xx+yy)],
        ]

    @staticmethod
    def _q_normalize(q):
        w, x, y, z = q
        n2 = w*w + x*x + y*y + z*z
        if n2 <= 0.0:
            return (1.0, 0.0, 0.0, 0.0)
        
        n = math.sqrt(n2)
        return (w/n, x/n, y/n, z/n)

    @staticmethod
    def _twos16(h, l):
        v = (h << 8) | l
        return v - 0x10000 if v & 0x8000 else v

    @staticmethod
    def _twos32(b0, b1, b2, b3):
        v = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3
        if v & 0x80000000:
            v = -((~v & 0xFFFFFFFF) + 1)
        return v