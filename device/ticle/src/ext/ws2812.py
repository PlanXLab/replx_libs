...
class WS2812Matrix:
    ...
    FONT_PATH = "lib/ticle/font/"
    DEFAULT_FONT: str = "vga2_bold_16x16"
    ...
    def __init__(self, pin_sm_pairs: list[tuple[int, int]], *,
        ...
        self._panels_per_sm: int = math.ceil((grid_width * grid_height) / len(pin_sm_pairs))
        ...
        for pin_no, sm_id in pin_sm_pairs:
            try:
                ...
                sm = rp2.StateMachine(sm_id, _ws2812_pio, freq=8_000_000, sideset_base=pin)
                sm.active(1)
                self._sms.append(sm)
                buf_len = self._pixels_per_panel * self._panels_per_sm
                self._tx_bufs.append(array.array("I", [0] * buf_len))
            except OSError as e:
        ...
        self._sm_ids: list[int] = [sm_id for _, sm_id in pin_sm_pairs]
    ...
    def brightness(self, value: float) -> None:
        ...
        self.__btab = bytes(int(i * self._brightness + 0.5) for i in range(256))
    ...
    def shader_split_lr(self, c_left, c_right, ratio_num=1, ratio_den=2):
        ...
        return (self._SH_SPLIT_LR, self._pack_grb(r1,g1,b1), self._pack_grb(r2,g2,b2), int(ratio_num), int(ratio_den))
    ...
    def shader_split_tb(self, c_top, c_bottom, ratio_num=1, ratio_den=2):
        ...
        return (self._SH_SPLIT_TB, self._pack_grb(r1,g1,b1), self._pack_grb(r2,g2,b2), int(ratio_num), int(ratio_den))
        
    @micropython.native
    def shader_checker(self, c1, c2, cell_w=2, cell_h=2):
        r1,g1,b1 = self._normalize_color(c1)
        r2,g2,b2 = self._normalize_color(c2)
        return (self._SH_CHECKER, self._pack_grb(r1,g1,b1), self._pack_grb(r2,g2,b2), int(cell_w), int(cell_h))
        
    @micropython.native
    def shader_snowflake(self,c1, c2, arm=1):
        r1,g1,b1 = self._normalize_color(c1)
        r2,g2,b2 = self._normalize_color(c2)
        return (self._SH_SNOW, self._pack_grb(r1,g1,b1), self._pack_grb(r2,g2,b2), int(arm))
    ...
    def draw_text_scroll_blocking(self, text: str, 
        ...
        if direction in ("left", "right"):
            if mode == 'shader':
                self._scroll_text_h_shader(text, direction, fg_shader, bg, step_px, space_scale, right_margin, left_margin, speed_ms, update_flag, right_mirrored)
            else:
                self._scroll_text_h(text, direction, fg, bg, step_px, space_scale, right_margin, left_margin, speed_ms, update_flag, right_mirrored)
        elif direction in ("up", "down"):
                if mode == 'shader':
                    self._scroll_text_v_shader(text, direction, x, y, fg_shader, bg, step_px, space_scale, left_margin, right_margin, up_margin, down_margin, speed_ms, update_flag)
                else:
                    self._scroll_text_v(text, direction, x, y, fg, bg, step_px, space_scale, left_margin, right_margin, up_margin, down_margin, speed_ms, update_flag)
        else:
    ...
    def draw_text_scroll(self, text: str, 
        ...
        self._sc_period = max(1, int(speed_ms) if speed_ms > 0 else 1)
        self._sc_chars = list(text)
    ...
    def _sc_tick_irq(self, _t):
        if not self._sc_posted:
            self._sc_posted = 1
            try:
                micropython.schedule(self._sc_step_sched, None)
            except:
                self._sc_posted = 0

    def _sc_step_sched(self, _):
        self._sc_posted = 0
    ...
    def _sc_h_step(self) -> bool:
        if self._sc_remain == 0 and self._sc_tail_rem == 0:
            ...
            if not ok and self._sc_tail_rem == 0:
    ...
    @micropython.viper
    def _glyph_row_bits(self, gi: int, rr: int) -> int:
    ...
    @micropython.viper
    def _glyph_lr_compute_packed(self, off: int) -> int:
        fw: int = int(self.font_width)
    ...
    def _present_from_vb(self, head: int, wait_complete: int):
        ...
        bufs = self._tx_bufs   # list[array('I')]
        dmas = self._dmas      # list[DMA]
        sms = self._sms        # list[StateMachine]
        sm_ids = self._sm_ids  # list[int]
    ...
    @micropython.native
    def _glyph_lr(self, off: int) -> tuple[int, int]:
        t = self._blank_cache.get(off)
        if t is not None:
            return t

        packed = self._glyph_lr_compute_packed(off)
        blank_L = packed >> 16
        blank_R = packed & 0xFFFF
        t = (blank_L, blank_R)
        self._blank_cache[off] = t
        return t
    ...
    def _load_font(self, font_src: str | object) -> None:
        ...
        if isinstance(font_src, str):
            font_src = self.FONT_PATH + font_src
    ...
    @micropython.native
    def _glyph_offset_sparse(self, code: int) -> int:
        lo = 0
    ...
    @micropython.native
    def _glyph_offset(self, code: int) -> int:
        if self._sparse_font:
            off = self._glyph_offset_sparse(code)
            if off >= 0:
                return off

            off = self._glyph_offset_sparse(self._fallback_cp)
            return off
        else:
    ...
    def _cached_block_width(self, ch: str, *, space_scale: float, left_margin: int, right_margin: int) -> int:
        ...
        if bw is None:
            ...
            if len(self._ink_width_cache) > 128:
                for k in list(self._ink_width_cache)[:64]:
                    self._ink_width_cache.pop(k, None)
    ...
    def _fg_mode(self, fg, text: str, chars: list):
        ...
        if isinstance(fg, (list, tuple)) and fg and isinstance(fg[0], (list, tuple)):
            fg_tbl = [self._pack_grb(*c) for c in fg]
        ...
        if callable(fg):
            try:
                ...
                fg_tbl = [self._pack_grb(*self._normalize_color(fg(i, text))) for i, _ in enumerate(chars)]
                return 'per_char', fg_tbl, 0, None
            except TypeError:
                return 'shader', None, 0, fg
    ...
    def _scroll_h_advance(self, head:int, write_x:int, disp_w:int, rb_w:int, direction:str, shift:int, speed_ms:int, update_flag:bool, packed_bg:int):
        ...
        if direction == "left":
            for s in range(shift):
                self._rb_fill_col((head + s) % rb_w, packed_bg)

            head    = (head + shift)   % rb_w
            write_x = (write_x + shift) % rb_w
        else:
            for s in range(shift):
                self._rb_fill_col((head + disp_w - 1 - s) % rb_w, packed_bg)
    ...
    def _h_tail_cleanup(self, head:int, direction:str, step_px:int, packed_bg:int):
        ...
        if direction == "left":
            ...
            for i in range(n):
                self._rb_fill_col((start - i) % rb_w, packed_bg)
        else:
            start = head % rb_w
            for i in range(n):
                self._rb_fill_col((start + i) % rb_w, packed_bg)
        ...
        for idx, ch in enumerate(chars):
            ...
            if gi >= 0:
                if is_spec and int(fg_shader[0]) == int(self._SH_SPLIT_TB):
                    ...
                    ink_l = blank_L
                    ink_r = fw - 1 - blank_R
                    self._scroll_h_blit_glyph_split_tb_v(gi, fw, fh, rb_w, write_x, gw, left_margin, dir_right, 1 if right_mirrored else 0, thr_tb, ink_l, ink_r, fg_shader[1], fg_shader[2])
                elif is_spec:
