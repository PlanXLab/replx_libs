...
class Servos:
    ...
    class _View:
        def __init__(self, parent: "Servos", indices: list[int]):
            self._parent = parent
            self._indices = indices

        def __getitem__(self, idx: int|slice) -> "Servos._View":
            if isinstance(idx, slice):
                selected_indices = [self._indices[i] for i in range(*idx.indices(len(self._indices)))]
                return Servos._View(self._parent, selected_indices)
            else:
                return Servos._View(self._parent, [self._indices[idx]])
        ...
        def angle(self) -> list[float]:
            return Servos._get_angle_list(self._parent, self._indices)
        ...
        def angle(self, value: float | list[float]):
            if isinstance(value, (list, tuple)):
                ...
                for idx, deg in zip(self._indices, value):
                    ...
                    Servos._set_angle_single(self._parent, idx, deg)
            else:
                deg = float(value)
                if not (0.0 <= deg <= 180.0):
                    raise ValueError("Angle must be between 0.0 and 180.0 degrees")
                for i in self._indices:
                    Servos._set_angle_single(self._parent, i, deg)

        @property
        def target_angle(self) -> list[float]:
            return Servos._get_target_angle_list(self._parent, self._indices)

        @property
        def nonblocking(self) -> list[bool]:
            return Servos._get_nonblocking_list(self._parent, self._indices)

        @nonblocking.setter
        def nonblocking(self, flag: bool):
            Servos._set_nonblocking_all(self._parent, flag, self._indices)

        @property
        def is_moving(self) -> list[bool]:
            return Servos._get_is_moving_list(self._parent, self._indices)

        @property
        def duration_ms(self) -> list[int]:
            return Servos._get_duration_ms_list(self._parent, self._indices)

        @duration_ms.setter
        def duration_ms(self, ms: int):
            Servos._set_duration_ms_all(self._parent, ms, self._indices)

        @property
        def calibration(self) -> list[dict]:
            return Servos._get_calibration_list(self._parent, self._indices)

        @calibration.setter
        def calibration(self, params: dict):
            Servos._set_calibration_all(self._parent, params, self._indices)

        def wait_completion(self, timeout_ms: int = 10000) -> bool:
            return Servos._wait_completion_all(self._parent, self._indices, timeout_ms)

        def stop(self):
            Servos._stop_all(self._parent, self._indices)
