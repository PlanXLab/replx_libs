...
class SR04s:
    ...
    class _View:
        ...
        def __init__(self, parent: "SR04s", indices: list[int]):
            self._parent = parent
            self._indices = indices

        def __getitem__(self, idx: int|slice) -> "SR04s._View":
            if isinstance(idx, slice):
                selected_indices = [self._indices[i] for i in range(*idx.indices(len(self._indices)))]
                return SR04s._View(self._parent, selected_indices)
            else:
                return SR04s._View(self._parent, [self._indices[idx]])
