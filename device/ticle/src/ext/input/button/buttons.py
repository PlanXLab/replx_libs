...
class Buttons:
    ...
    def __init__(self, pins: list[int] | tuple[int, ...]):
        ...
        self._active_type = [Buttons.ACTIVE_HIGH] * n
        self._last = [0] * n
    ...
    def __getitem__(self, idx: int | slice) -> "_View":
        if isinstance(idx, slice):
            ...
            return Buttons._View(self, indices)
        elif isinstance(idx, int):
            if not (0 <= idx < len(self._pins)):
                raise IndexError("Buttons index out of range")
            return Buttons._View(self, [idx])
        else:
    ...
    def _button_interrupt_handler(self, pin_num: int, rising: bool) -> None:
        try:
            ...
            if active_type == Buttons.ACTIVE_HIGH:
    ...
    def _get_click_list(parent, indices: list[int]) -> list[bool]:
        ...
        for idx in indices:
            ...
            if parent._active_type[idx] == Buttons.ACTIVE_HIGH:
    ...
    class _View:
        def __init__(self, parent: "Buttons", indices: list[int]):
            self._parent = parent
            self._indices = indices

        def __getitem__(self, idx: int | slice) -> "Buttons._View":
            if isinstance(idx, slice):
                selected_indices = [self._indices[i] for i in range(*idx.indices(len(self._indices)))]
                return Buttons._View(self._parent, selected_indices)
            else:
                return Buttons._View(self._parent, [self._indices[idx]])
        ...
        def active_type(self) -> list[bool]:
            return Buttons._get_active_type_list(self._parent, self._indices)

        @active_type.setter
        def active_type(self, logic_type: bool):
            Buttons._set_active_type_all(self._parent, logic_type, self._indices)

        @property
        def measurement(self) -> list[bool]:
            return Buttons._get_measurement_list(self._parent, self._indices)

        @measurement.setter
        def measurement(self, enabled: bool):
            Buttons._set_measurement_all(self._parent, enabled, self._indices)

        @property
        def debounce_ms(self) -> list[int]:
            return Buttons._get_debounce_ms_list(self._parent, self._indices)

        @debounce_ms.setter
        def debounce_ms(self, ms: int):
            Buttons._set_debounce_ms_all(self._parent, ms, self._indices)

        @property
        def double_click_window_ms(self) -> list[int]:
            return Buttons._get_double_click_window_ms_list(self._parent, self._indices)

        @double_click_window_ms.setter
        def double_click_window_ms(self, ms: int):
            Buttons._set_double_click_window_ms_all(self._parent, ms, self._indices)

        @property
        def long_press_threshold_ms(self) -> list[int]:
            return Buttons._get_long_press_threshold_ms_list(self._parent, self._indices)

        @long_press_threshold_ms.setter
        def long_press_threshold_ms(self, ms: int):
            Buttons._set_long_press_threshold_ms_all(self._parent, ms, self._indices)
        ...
        def pressed(self) -> list[bool]:
            ...
            for i in self._indices:
                ...
                if self._parent._active_type[i] == Buttons.ACTIVE_HIGH:
        ...
        def click(self) -> list[bool]:
            return Buttons._get_click_list(self._parent, self._indices)

        @property
        def on_clicked(self) -> list[callable]:
            return Buttons._get_on_clicked_list(self._parent, self._indices)

        @on_clicked.setter
        def on_clicked(self, callback: callable):
            Buttons._set_on_clicked_all(self._parent, callback, self._indices)

        @property
        def on_double_clicked(self) -> list[callable]:
            return Buttons._get_on_double_clicked_list(self._parent, self._indices)

        @on_double_clicked.setter
        def on_double_clicked(self, callback: callable):
            Buttons._set_on_double_clicked_all(self._parent, callback, self._indices)

        @property
        def on_long_pressed(self) -> list[callable]:
            return Buttons._get_on_long_pressed_list(self._parent, self._indices)

        @on_long_pressed.setter
        def on_long_pressed(self, callback: callable):
            Buttons._set_on_long_pressed_all(self._parent, callback, self._indices)

        @property
        def on_pressed(self) -> list[callable]:
            return Buttons._get_on_pressed_list(self._parent, self._indices)

        @on_pressed.setter
        def on_pressed(self, callback: callable):
            Buttons._set_on_pressed_all(self._parent, callback, self._indices)

        @property
        def on_released(self) -> list[callable]:
            return Buttons._get_on_released_list(self._parent, self._indices)

        @on_released.setter
        def on_released(self, callback: callable):
            Buttons._set_on_released_all(self._parent, callback, self._indices)
