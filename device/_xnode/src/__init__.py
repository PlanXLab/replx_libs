        ...
        for i in range(0, 8):
            ...
            for j in range(0, 16):
                ...
                if address in devices:
                    print(Term.FG.BRIGHT_YELLOW + " {:02x}".format(address) + Term.RESET, end='')
                else:
                    print(" --", end='')
            print()
...
class Illuminance:
    ...
    def __init__(self, *, mode=CONT_HIGH_RES):
        self.__addr = Illuminance.BH1750_ADDR
        self.__i2c = machine.I2C(1)

        self.init(mode)

    def init(self, mode):
        self.__i2c.writeto(self.__addr, bytes([self.POWER_ON]))
        self.__i2c.writeto(self.__addr, bytes([self.POWER_RESET]))
        self.__i2c.writeto(self.__addr, bytes([mode])) 
 
    def deinit(self):
        self.__i2c.writeto(self.__addr, bytes([self.POWER_OFF]))

    def read(self):      
        data = self.__i2c.readfrom(self.__addr, 2)
...
class Tphg:
    ...
    def __set_power_mode(self, value):
        tmp = self.__i2c.readfrom_mem(self.__addr, 0x74, 1)[0] 
        tmp &= ~0x03
        tmp |= value
        self.__i2c.writeto_mem(self.__addr, 0x74, bytes([tmp]))
    ...
    def __perform_reading(self):
        ...
        while gas_measuring and utime.ticks_diff(timeout_time, utime.ticks_ms()) > 0:
            data = self.__i2c.readfrom_mem(self.__addr, 0x1D, 1)
        ...
        data = self.__i2c.readfrom_mem(self.__addr, 0x1D, 17)
    ...
    def __init__(self, temp_weighting=0.10,  pressure_weighting=0.05, humi_weighting=0.20, gas_weighting=0.65, gas_ema_alpha=0.1, temp_baseline=23.0,  pressure_baseline=1013.25, humi_baseline=45.0, gas_baseline=450_000):
        self.__addr = Tphg.BME680_ADDR
        self.__i2c = machine.I2C(1)
        
        self.__i2c.writeto_mem(self.__addr, 0xE0, bytes([0xB6]))                         # Soft reset
        utime.sleep_ms(5)        
          
        self.__set_power_mode(Tphg.SLEEP_MODE)
        
        t_calibration = self.__i2c.readfrom_mem(self.__addr, 0x89, 25)
        t_calibration += self.__i2c.readfrom_mem(self.__addr, 0xE1, 16)
        
        self._sw_err = (self.__i2c.readfrom_mem(self.__addr, 0x04, 1)[0] & 0xF0) / 16
        ...
        self.__i2c.writeto_mem(self.__addr, 0x72, bytes([0b001]))                        # Humidity oversampling x1
        self.__i2c.writeto_mem(self.__addr, 0x74, bytes([(0b010 << 5) | (0b011 << 2)]))  # Temperature oversampling x2, Pressure oversampling x4
        self.__i2c.writeto_mem(self.__addr, 0x75, bytes([0b001 << 2]))                   # Filter coefficient 3 (only to temperature and pressure data)
        
        self.__i2c.writeto_mem(self.__addr, 0x50, bytes([0x1F]))                         # idac_heat_0
        self.__i2c.writeto_mem(self.__addr, 0x5A, bytes([0x73]))                         # res_heat_0
        self.__i2c.writeto_mem(self.__addr, 0x64, bytes([0x64]))                         # gas_wait_0 is 100ms (1ms ~ 4032ms, 20ms ~ 30ms are neccessary)
                
        self.__i2c.writeto_mem(self.__addr, 0x71, bytes([(0b1 << 4) | (0b0000)]))        # run_gas(enable gas measurements), nv_conv (index of heater set-point 0)
        utime.sleep_ms(50)
