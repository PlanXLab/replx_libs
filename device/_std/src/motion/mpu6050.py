...
_DMP_ACC_LSB_PER_G  = 2048.0                # DMP packet accel scale
_G0                 = 9.80665               # [m/s^2]
_GYR_LSB_PER_DPS    = 16.4                  # ±2000 dps
_DPS_TO_RAD_S       = 0.017453292519943295  # math.pi / 180.0
_DMP_PACKET_SIZE    = const(42)             # include quat (16 bytes)
...
class MPU6050:
    ...
    def __init__(self, i2c, *, addr=0x68, mode=Mode.RAW_BALANCED, coord='raw', remap=None):
        ...
        self._acc_bias = [0.0, 0.0, 0.0]  # m/s^2
        self._gyr_bias = [0.0, 0.0, 0.0]  # rad/s
        self._gyr_raw_bias = [0.0, 0.0, 0.0]
        self._dmp_accel = (0.0, 0.0, 0.0) # DMP packet accel cache (m/s^2)
    ...
    def quat(self):
        ...
        # Extract accel from DMP packet for frame-synchronized tilt/linear.
    ...
    def linear(self):
        ...
        q = self.quat  # Updates _dmp_accel from same DMP packet
    ...
    def _quick_bias(self, n=200, delay_ms=2):
        ...
        # Auto-detect dominant gravity axis at startup time.
        # Subtracts 1g (with correct sign) only from the axis that actually has gravity,
        # regardless of remap/coord setting or startup orientation.
        # This prevents wrong bias when the board is not in standard orientation at boot.
        vals = (axg, ayg, azg)
