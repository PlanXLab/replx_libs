...
# @depends: i2c
# @platforms: *
# @tags: bme680, bme688, temperature, pressure, humidity, gas, iaq, air quality
...
_FORCED_MODE         = const(0x01)
_SLEEP_MODE          = const(0x00)
_REG_STATUS          = const(0x1D)
_BIT_MEASURING       = const(0x20)
_REG_CTRL_GAS1       = const(0x71)
_REG_RES_HEAT_0      = const(0x5A)
_REG_GAS_WAIT_0      = const(0x64)
_BIT_NEW_DATA        = const(0x80)
_BIT_HEAT_STAB       = const(0x10)
_BIT_GAS_VALID       = const(0x20)
_DATA_BLOCK_LEN      = const(17)
_NORM_BASE_PERIOD_MS = const(3000)
...
_IIR_COEFFS  = (0, 1, 3, 7, 15, 31, 63, 127)
...
class BME68x:
    ...
    def __init__(self, sda: int, scl: int, *, addr: int = 0x77,
        ...
        self._set_power_mode(_SLEEP_MODE)
        ...
        par_t2 = struct.unpack('<h', cal1[0:2])[0]
        par_t3 = struct.unpack('b',  cal1[2:3])[0]
        par_t1 = struct.unpack('<H', cal2[8:10])[0]
        self.__par_t1, self.__par_t2, self.__par_t3 = par_t1, par_t2, par_t3

        # Pressure calibration
        par_p1 = struct.unpack('<H', cal1[4:6])[0]
        par_p2 = struct.unpack('<h', cal1[6:8])[0]
        par_p3 = struct.unpack('b',  cal1[8:9])[0]
        par_p4 = struct.unpack('<h', cal1[10:12])[0]
        par_p5 = struct.unpack('<h', cal1[12:14])[0]
        par_p7 = struct.unpack('b',  cal1[14:15])[0]
        par_p6 = struct.unpack('b',  cal1[15:16])[0]
        par_p8 = struct.unpack('<h', cal1[18:20])[0]
        par_p9 = struct.unpack('<h', cal1[20:22])[0]
        par_p10 = cal1[22]
        ...
        # signed byte conversion: v - 256 if v > 127 else v
        par_h3 = cal2[3] - 256 if cal2[3] > 127 else cal2[3]
        par_h4 = cal2[4] - 256 if cal2[4] > 127 else cal2[4]
        par_h5 = cal2[5] - 256 if cal2[5] > 127 else cal2[5]
        par_h6 = cal2[6]
        par_h7 = cal2[7] - 256 if cal2[7] > 127 else cal2[7]
        self._humidity_calibration = [par_h1, par_h2, par_h3, par_h4, par_h5, par_h6, par_h7]

        # Gas/Heater calibration
        par_gh2 = struct.unpack('<h', cal2[10:12])[0]
        par_gh1 = struct.unpack('b',  cal2[12:13])[0]
        par_gh3 = struct.unpack('b',  cal2[13:14])[0]
        self.__par_gh1, self.__par_gh2, self.__par_gh3 = par_gh1, par_gh2, par_gh3

        res_heat_val = cal3[0] - 256 if cal3[0] > 127 else cal3[0]
    ...
    def gas_update_hint_ms(self, value: int):
        ...
        if self._heater_sig != (rh, gw):
            self._i2c.writeto_mem(self._addr, _REG_RES_HEAT_0, bytes([rh]))
            self._i2c.writeto_mem(self._addr, _REG_GAS_WAIT_0, bytes([gw]))
    ...
    def set_profile_preset(self, preset: str):
        ...
        if p == "eco":
            ...
            self.gas_update_hint_ms = _NORM_BASE_PERIOD_MS  # 3s
    ...
    def set_gas_heater_profiles(self, temps_c: list, durations_ms: list, amb_temp_c: float = None):
        ...
        for i, (tc, dm) in enumerate(zip(tlist, dlist)):
            ...
            self._i2c.writeto_mem(self._addr, _REG_RES_HEAT_0 + i, bytes([rh]))
            self._i2c.writeto_mem(self._addr, _REG_GAS_WAIT_0 + i, bytes([gw]))
    ...
    def set_gas_nb_conv(self, n: int = 0):
        n = max(0, min(9, int(n)))
        val = self._i2c.readfrom_mem(self._addr, _REG_CTRL_GAS1, 1)[0]
        # keep run_gas(bit4), replace nb_conv(low 4 bits)
        val = (val & ~0x0F) | n
        self._i2c.writeto_mem(self._addr, _REG_CTRL_GAS1, bytes([val]))
    ...
    def start(self, *, gas: bool = False) -> int:
        ...
        if gas:
            ...
            self._set_run_gas(False)
        self._set_power_mode(_FORCED_MODE)
        return self._estimate_eta_ms(need_gas=gas)

    def ready(self) -> bool:
        st = self._i2c.readfrom_mem(self._addr, _REG_STATUS, 1)[0]
        return bool(st & _BIT_NEW_DATA)

    def result(self) -> tuple:
        self._i2c.readfrom_mem_into(self._addr, _REG_STATUS, self._buf17)
        ctrl = self._i2c.readfrom_mem(self._addr, _REG_CTRL_GAS1, 1)[0]
        need_gas = (ctrl & 0x10) != 0
        self._parse_and_cache_sample(self._buf17, assume_need_gas=need_gas)
        if need_gas and self._ready_gas:
            return (self._temp_c, self._pres_hpa, self._humi_rh, self._gas_ohm)
        return (self._temp_c, self._pres_hpa, self._humi_rh)

    def read(self, *, gas: bool = False) -> tuple:
        self.start(gas=gas)
        eta_ms = self._estimate_eta_ms(need_gas=gas)
        deadline = time.ticks_add(time.ticks_ms(), eta_ms + 100)
        while time.ticks_diff(deadline, time.ticks_ms()) > 0:
            if self.ready():
                return self.result()
            time.sleep_ms(5)
        raise OSError("BME68x measurement timeout")
    ...
    def burn_in(self, mode: str = "simple"):
        ...
        while time.ticks_diff(time.ticks_ms(), start_ms) < max_minutes * 60_000:
            ...
            if g is not None:
                ...
                if n < win_n:
                    med_partial, cov_partial = self._median_cov(xs, n)
                    ...
                    if consecutive_ok >= windows_needed:
                        ...
                        yield {
                            ...
                            "median_ohm": med,
                            "baseline_ohm": self._gas_baseline,
                            "mode": mode,
                        }
                        return
    ...
    def iaq_heuristics(self, *,
            ...
            humi_weighting=0.15, gas_weighting=0.70,
            gas_ema_alpha=0.05,
            temp_opt_low=18.0, temp_opt_high=26.0,
            humi_opt_low=40.0, humi_opt_high=60.0,
            pressure_baseline=1013.25
            ):
        eta_ms = self.trigger_gas_measurement()
        deadline = time.ticks_add(time.ticks_ms(), eta_ms + 20)
        while time.ticks_diff(deadline, time.ticks_ms()) > 0:
            if self.gas_measurement_ready():
                break
            time.sleep_ms(3)
        ...
        now = time.ticks_ms()
        temp_mid = (temp_opt_low + temp_opt_high) / 2.0
        humi_mid = (humi_opt_low + humi_opt_high) / 2.0
        near_env = (temp_opt_low <= temp <= temp_opt_high) and (humi_opt_low <= humi <= humi_opt_high)
        cleaner = (gas >= self._gas_baseline * 0.95)
        elapsed = time.ticks_diff(now, self._iaq_last_baseline_update_ms)
        if near_env and cleaner and rising and elapsed >= self._gas_baseline_auto_update_ms:
            self._gas_baseline = (1.0 - gas_ema_alpha) * self._gas_baseline + gas_ema_alpha * gas
            self._iaq_last_baseline_update_ms = now

        if temp < temp_opt_low:
            temp_dev = (temp_opt_low - temp) / 10.0
        elif temp > temp_opt_high:
            temp_dev = (temp - temp_opt_high) / 10.0
        else:
            temp_dev = 0.0
        temp_bad = min(temp_dev, 1.0) * (temp_weighting * 100.0)

        if humi < humi_opt_low:
            humi_dev = (humi_opt_low - humi) / 40.0
        elif humi > humi_opt_high:
            humi_dev = (humi - humi_opt_high) / 40.0
        else:
            humi_dev = 0.0
        hum_bad = min(humi_dev, 1.0) * (humi_weighting * 100.0)

        pres_dev = abs(pres - pressure_baseline) / 30.0
        pres_bad = min(pres_dev, 1.0) * (pressure_weighting * 100.0)

        if gas >= self._gas_baseline:
            gas_score = 0.0
        else:
            ratio = gas / max(self._gas_baseline, 1.0)
            ratio = max(0.1, min(ratio, 1.0))
            gas_score = 1.0 - math.pow(ratio, 0.6)
        gas_bad = gas_score * (gas_weighting * 100.0)

        raw = hum_bad + temp_bad + pres_bad + gas_bad
        iaq_val = int(min(max(5.0 * raw, 0.0), 500.0))
    ...
    def _calc_tph_time_ms_from_codes(self, osrs_t: int, osrs_p: int, osrs_h: int) -> int:
        ot = _OSRS_MULT[osrs_t] if 0 <= osrs_t <= 5 else 0
        op = _OSRS_MULT[osrs_p] if 0 <= osrs_p <= 5 else 0
        oh = _OSRS_MULT[osrs_h] if 0 <= osrs_h <= 5 else 0
    ...
    def _perform_reading(self, need_gas: bool):
        ...
        cg = self._i2c.readfrom_mem(self._addr, _REG_CTRL_GAS1, 1)[0]
        was_on = (cg & 0x10) != 0
        nb_conv = (cg & 0x0F)  # 0..9
        ...
        def _wait_once(extra_margin=0):
            ...
            t0 = time.ticks_add(time.ticks_ms(), 5)
            while time.ticks_diff(t0, time.ticks_ms()) > 0:
                st = self._i2c.readfrom_mem(self._addr, _REG_STATUS, 1)[0]
                if st & _BIT_MEASURING:
                    break

            deadline = time.ticks_add(time.ticks_ms(), t_est_ms + margin_ms + extra_margin)
            while time.ticks_diff(deadline, time.ticks_ms()) > 0:
                st = self._i2c.readfrom_mem(self._addr, _REG_STATUS, 1)[0]
                if st & _BIT_NEW_DATA:
                    return True
    ...
    def _parse_and_cache_sample(self, data: bytes, *, assume_need_gas: bool):
        ...
        self._adc_hum  = (mv[8] << 8) | mv[9]
        self._adc_gas  = ((mv[13] << 8) | mv[14]) >> 6
        ...
        if assume_need_gas:
            gas_valid = (flags & _BIT_GAS_VALID) != 0
            heat_ok = ((flags & _BIT_HEAT_STAB) != 0) or (not self._require_heat_stab)
        ...
        self._cache_ts_ms = time.ticks_ms()
    ...
    def _set_run_gas(self, enable: bool):
        ...
        if new != val:
            self._i2c.writeto_mem(self._addr, _REG_CTRL_GAS1, bytes([new]))
        return new
    ...
    def _gas(self):
        ...
        var1 = ((1340 + (5 * rs_err)) * _GAS_LUT1[gr]) >> 16
        var2 = (((adc << 15) - 16777216) + var1)
        var3 = ((_GAS_LUT2[gr] * var1) >> 9)
        var3 += (var2 >> 1)
